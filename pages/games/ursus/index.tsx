...
import { matrix } from "@/pages/toolbox/array"
...
class Ursus extends Component<{}, State> {
  ...
  round() {
    if (!this.state.active) {
      ...
      this.setState({
        next: shapeCollections.random().random(),
        active: newActive,
      })
      return
    }
    try {
      ...
      if (e === ERR_CELL_NOT_EMPTY || e === ERR_OUT_OF_PANEL) {
        ...
        const completedRows = [...new Set(this.state.active.blocks.map(block => block.y))].filter(row => {
  ...
  override render() {
    const panel = this.state.active ? this.state.active.print(this.state.solidPanel) : this.state.solidPanel

    //todo UI for showing next shape
    return (
      <div className={styles.container}>
        <div className={styles.next}>
          {JSON.stringify(this.state.next)}
        </div>
        ...
        <div className={styles.score}>
          <span className={styles.scoreText}>{this.state.score.toString().padStart(8, "0")}</span>
