...
function initMatrix<T>({ width, height, defaultValue }: MatrixProps<T>): T[][] {
  return new Array<T[]>(height).fill(new Array<T>(width)).map(() => {
    const row: T[] = []
    row.length = width
...
const SpeedMarks = Object.entries(Speeds).map(([k, v]) => ({
  value: Number(k),
  label: v,
}))
...
function evolute(world: boolean[][]): boolean[][] {
  ...
  for (let i = 0; i < previous_row.length; i++) {
    ...
    if (
      previous_row[i] &&
      (live_neighbor_count > 3 || live_neighbor_count < 2)
    ) {
  ...
  for (let r = 1; r < world.length - 1; r++) {
    const row_tmp = [...world[r]]
    for (let c = 0; c < row_tmp.length; c++) {
      let live_neighbor_count =
        Number(world[r - 1][c]) + Number(world[r + 1][c])
      if (c > 0) {
